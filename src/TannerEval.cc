...
{
  m_hits_ID = new int[m_maxNHits];
  m_hits_layerID = new float[m_maxNHits];
  m_hits_layertrueID = new float[m_maxNHits];
  m_hits_trueID = new float[m_maxNHits];
  m_hits_x = new float[m_maxNHits];
  m_hits_y = new float[m_maxNHits];
  m_hits_z = new float[m_maxNHits];
  m_hits_true_x = new float[m_maxNHits];
  m_hits_true_y = new float[m_maxNHits];
  m_hits_true_z = new float[m_maxNHits];
...
int TannerEval::Init(PHCompositeNode* /*topNode*/){
    ...
    if (m_doEventInfo){
       ...
       m_EventTree->Branch("m_n_generator_accepted", &m_n_generator_accepted, "m_n_generator_accepted/I");
     }
    ...
    if (m_doHits){
       m_EventTree->Branch("m_nHits", &m_nHits, "m_nHits/I");
       m_EventTree->Branch("m_hits_ID", m_hits_ID, "m_hits_ID[m_nHits]/I");
       m_EventTree->Branch("m_hits_layerID", m_hits_layerID, "m_hits_layerID[m_nHits]/F");
       m_EventTree->Branch("m_hits_layertrueID", m_hits_layertrueID, "m_hits_layertrueID[m_nHits]/F");
       m_EventTree->Branch("m_hits_trueID", m_hits_trueID, "m_hits_trueID[m_nHits]/F");
       ...
       m_EventTree->Branch("m_hits_true_x", m_hits_true_x, "m_hits_true_x[m_nHits]/F");
       m_EventTree->Branch("m_hits_true_y", m_hits_true_y, "m_hits_true_y[m_nHits]/F");
       m_EventTree->Branch("m_hits_true_z", m_hits_true_z, "m_hits_true_z[m_nHits]/F");
...
void TannerEval::fillOutputNtuples(PHCompositeNode* topNode){
  ...
  if(!tgeometry or !surfmaps){
    std::cout << "No Acts geometry on node tree. Can't  continue."<< std::endl;
    return;
  }

  ActsTransformations actsTransformer;

  SvtxTruthEval* trutheval = m_SVTX_EvalStack->get_truth_eval();
  SvtxClusterEval* clustereval = m_SVTX_EvalStack->get_cluster_eval();
  if(m_doHits){
    if (Verbosity() > 0)
    {
    ...
    for (std::set<PHG4Hit*>::iterator iter = g4hits.begin(); iter != g4hits.end(); ++iter) {
      ...
      //  PHG4Particle* g4particle = trutheval->get_particle(g4hit);
        m_hits_ID[m_nHits] = m_nHits;
        m_hits_true_x[m_nHits] = g4hit->get_avg_x();
        m_hits_true_y[m_nHits]= g4hit->get_avg_y();
        m_hits_true_z[m_nHits] = g4hit->get_avg_z();
        m_hits_layertrueID[m_nHits]= g4hit->get_layer();
        m_hits_trueID[m_nHits] = g4hit->get_hit_id();
        ...
        if(!clustermap) { clustermap = findNode::getClass<TrkrClusterContainer>(topNode, "TRKR_CLUSTER"); }
    ...
    auto global = actsTransformer.getGlobalPosition(cluster,surfmaps,tgeometry);
          m_hits_x[m_nHits] = global(0);
          m_hits_y[m_nHits] = global(1);
          m_hits_z[m_nHits] = global(2);
          // TVector3 vec2(x, y, z);
          // eta = vec2.Eta();
          // phi = vec2.Phi();
          // e = cluster->getAdc();
          // adc = cluster->getAdc();
          m_hits_layerID[m_nHits] = (float) TrkrDefs::getLayer(cluster_key);
        //  size = 0.0;
    ...
    //       {
    //         cout << "Layer \t" << m_nHits << "\t" << hit_iter->second->get_layer()<< "IN Region\t" << GetProjectionNameFromIndex(iIndex) <<endl;
    //         if (Verbosity() > 1) { cout << __PRETTY_FUNCTION__ << " found hit with id " << hit_iter->second->get_trkid() << endl;   }
    //
    //         if(m_nHits > m_maxNHits){
    //           cout << __PRETTY_FUNCTION__ << " exceededed maximum hit array size! Please check where these hits come from!" << endl;
    //           break;
    //         }
    //
    //         m_hits_x[m_nHits] = hit_iter->second->get_x(0);
    //         m_hits_y[m_nHits] = hit_iter->second->get_y(0);
    //         m_hits_z[m_nHits] = hit_iter->second->get_z(0);
    //         m_hits_t[m_nHits] = hit_iter->second->get_t(0);
    //         m_hits_layerID[m_nHits] = hit_iter->second->get_layer();
    ...
    //             {
    //               m_hits_trueID[m_nHits] = hit_iter->second->get_trkid();
    //             }
    //             else
    //             {
    ...
    //                   break;
    //                 } else {
    //                   m_hits_trueID[m_nHits] = g4particleMother2->get_parent_id();
    //                   mcSteps2 += 1;
    //                 }
    //               }
    //             }
    ...
    //       if (Verbosity() > 0)
    //       {
    //         cout << "saved\t" << m_nHits << "\thits for " << GetProjectionNameFromIndex(iIndex) << endl;
    //       }
    //     }
    //     else
    //     {
  ...
  if(m_doTracks){
      ...
      for (SvtxTrackMap::Iter iter = trackmap->begin(); iter != trackmap->end(); ++iter){
        ...
        //  track->get_cal_cluster_e(CAL_LAYER::CEMC);
      ...
      //    for (SvtxTrack::ConstStateIter trkstates = track->begin_states(); trkstates != track->end_states(); ++trkstates){
            ...
            cout << "GOT TOPROJECTIONS" << "'/t/t'"  << endl;

            //int trackStateIndex = GetProjectionIndex(trackStateName);
            if(m_doCEMC){
            // m_track_Proj_E[m_nProjections] = track->get_cal_cluster_e(SvtxTrack::CAL_LAYER::CEMC);
            float rCEMC = 95.0;
               //StvxTrack::CEMC;
            float thetaProj = 2*atan(exp(-1.0*(track->get_cal_deta(SvtxTrack::CEMC)+track->get_eta())));
            float phiProj = track->get_cal_dphi(SvtxTrack::CEMC)+track->get_phi();
            cout << track->get_cal_deta(SvtxTrack::CEMC) << "\t" << track->get_eta() << "\t" << track->get_x() << "\t" << track->get_y() << "\t" << track->get_z() << endl;
            cout << phiProj << "\t" << cos(phiProj) << "\t" << thetaProj << "\t"<<sin(thetaProj) <<endl;
            m_track_TLP_x[m_nProjections] = rCEMC*cos(phiProj)*sin(thetaProj);
            m_track_TLP_x[m_nProjections] = rCEMC*sin(phiProj)*sin(thetaProj);
            m_track_TLP_x[m_nProjections] = rCEMC*cos(thetaProj);
            m_track_ProjLayer[m_nProjections] = 6;
            m_track_ProjTrackID[m_nProjections] = m_nTracks;



               string nodename = "G4HIT_CEMC";
               PHG4HitContainer* hits = findNode::getClass<PHG4HitContainer>(topNode, nodename);
               if(hits){
                 PHG4HitContainer::ConstRange hit_range = hits->getHits();
                 for (PHG4HitContainer::ConstIterator hit_iter = hit_range.first; hit_iter != hit_range.second; hit_iter++){
                   if((hit_iter->second->get_trkid()-track->get_truth_track_id()) ==0){
                     m_track_TLP_true_x[m_nProjections] = hit_iter->second->get_x(0);
                     m_track_TLP_true_y[m_nProjections] = hit_iter->second->get_y(0);
                     m_track_TLP_true_z[m_nProjections] = hit_iter->second->get_z(0);
                     m_track_TLP_true_t[m_nProjections] = hit_iter->second->get_t(0);
                   }
                 }
               }
               m_nProjections++;
            ...
            if(m_doHCALIN){
              // m_track_Proj_E[m_nProjections] = track->get_cal_cluster_e(CAL_LAYER::HCALIN);
               float rHCALIN = 116.0;
              // float testLOL = track->get_cal_deta(SvtxTrack::HCALIN);
               float thetaProj = 2*atan(exp(-1.0*(track->get_cal_deta(SvtxTrack::HCALIN)+track->get_eta())));
               cout << thetaProj <<endl;
               float phiProj = track->get_cal_deta(SvtxTrack::HCALIN)+track->get_phi();
               cout << phiProj << endl;
                 cout << phiProj << "\t" << cos(phiProj) << "\t" << thetaProj << "\t"<<sin(thetaProj) << endl;
               m_track_TLP_x[m_nProjections] = rHCALIN*cos(phiProj)*sin(thetaProj);
               m_track_TLP_x[m_nProjections] = rHCALIN*sin(phiProj)*sin(thetaProj);
               m_track_TLP_x[m_nProjections] = rHCALIN*cos(thetaProj);
               m_track_ProjLayer[m_nProjections] = 5;
               m_track_ProjTrackID[m_nProjections] = m_nTracks;
               string nodename = "G4HIT_HCALIN";
               PHG4HitContainer* hits = findNode::getClass<PHG4HitContainer>(topNode, nodename);
               if(hits){
                 PHG4HitContainer::ConstRange hit_range = hits->getHits();
                 for (PHG4HitContainer::ConstIterator hit_iter = hit_range.first; hit_iter != hit_range.second; hit_iter++){
            ...
            if(m_doHCALOUT){
               //m_track_Proj_E[m_nProjections] = track->get_cal_cluster_e(CAL_LAYER::HCALOUT);
               float rHCALOUT = 183.3;
               float thetaProj = 2*atan(exp(-1.0*(track->get_cal_deta(SvtxTrack::HCALOUT)+track->get_eta())));
               float phiProj = track->get_cal_deta(SvtxTrack::HCALOUT)+track->get_phi();
               cout << phiProj << "\t" << cos(phiProj) << "\t" << thetaProj << "\t"<<sin(thetaProj) << endl;
               m_track_TLP_x[m_nProjections] = rHCALOUT*cos(phiProj)*sin(thetaProj);
               m_track_TLP_x[m_nProjections] = rHCALOUT*sin(phiProj)*sin(thetaProj);
               m_track_TLP_x[m_nProjections] = rHCALOUT*cos(thetaProj);
               m_track_ProjLayer[m_nProjections] = 7;
               m_track_ProjTrackID[m_nProjections] = m_nTracks;
               string nodename = "G4HIT_HCALOUT";
               PHG4HitContainer* hits = findNode::getClass<PHG4HitContainer>(topNode, nodename);
               if(hits){
                 PHG4HitContainer::ConstRange hit_range = hits->getHits();
                 for (PHG4HitContainer::ConstIterator hit_iter = hit_range.first; hit_iter != hit_range.second; hit_iter++){
...
if(m_doHits){
  ...
  for (int ihit = 0; ihit < m_maxNHits; ihit++)
  {
    ...
    m_hits_layertrueID[ihit] = 0;
    m_hits_trueID[ihit] = 0;
    m_hits_x[ihit] = 0;
    m_hits_y[ihit] = 0;
    m_hits_z[ihit] = 0;
    m_hits_true_x[ihit] = 0;
    m_hits_true_y[ihit] = 0;
    m_hits_true_z[ihit] = 0;
  }
