...
#include <tannereval/TannerEval.h>
...
R__LOAD_LIBRARY(libtannereval.so)

// For HepMC Hijing
// try inputFile = /sphenix/sim/sim01/sphnxpro/sHijing_HepMC/sHijing_0-12fm.dat

int Fun4All_TannerEval_sPHENIX(
    const int nEvents = 1,
...
//  if (Enable::JETS_EVAL) Jet_Eval(outputroot + "_g4jet_eval.root");
  ...
  TannerEval *tannereval = new TannerEval("TannerEval",outputroot + "_TannerEval.root");
  tannereval->Verbosity(1);

  tannereval->set_do_store_event_level_info(true);
  if (Enable::CEMC) tannereval->set_do_CEMC(true);
  if (Enable::HCALIN) tannereval->set_do_HCALIN(true);
  if (Enable::HCALOUT) tannereval->set_do_HCALOUT(true);
  tannereval->set_do_HITS(true);
  tannereval->set_do_TRACKS(true);
  tannereval->set_do_CLUSTERS(true);
  tannereval->set_do_VERTEX(true);
  tannereval->set_do_PROJECTIONS(true);
  tannereval->set_do_MCPARTICLES(true);
  //tannereval->set_do_HEPMC(true);
  tannereval->set_do_GEOMETRY(true);

  se->registerSubsystem(tannereval);
  //======================
